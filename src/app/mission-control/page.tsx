...
// TODO: paste your Firebase Web app config (NOT the admin key)
...
export default function MissionControl() {
  ...
  const handleAssign = async () => {
    ...
    const res = await fetch('/api/jobs/assign', {
      ...
      body: JSON.stringify({
        jobId: selected.wo?.id || selected.id,
        techId: assignTechId.trim()
      })
    });
  ...
  const handleDispatcherPreflight = async () => {
    if (!selected) return;
      ...
      <table cellPadding={8} style={{ width: '100%', borderCollapse: 'collapse' }}>
        <thead>
          <tr style={{ textAlign: 'left', borderBottom: '1px solid #e5e7eb' }}>
            <th>WO</th><th>Scope</th><th>Status</th><th>Chips</th><th>Assigned</th>
          </tr>
        </thead>
        <tbody>
          {rows.map((j) => {
            const pfReady = !!(j.preflight?.dispatcherDone && j.preflight?.techDone);
            const comp = j.compliance?.progress;
            const compText = comp ? `${comp.ok ?? 0}/${comp.required ?? 12}` : '0/12';
            const assigned = j.assignedTechs?.join(', ') || '—';

            return (
              <tr key={j.id} style={{ borderBottom: '1px solid #f3f4f6', cursor: 'pointer' }}
                  onClick={() => setSelected(j)}>
                <td>{j.wo?.id || j.id}</td>
                <td>{j.wo?.scope || '-'}</td>
                <td>{j.status}</td>
                <td>
                  <Chip ok={pfReady} text={`Pre-Flight ${pfReady ? 'Ready' : 'Missing'}`} />
                  <Chip ok={!!j.compliance?.ready} text={`Compliance ${compText}`} />
                </td>
                <td>{assigned}</td>
              </tr>
            );
      ...
      {selected && (
        <div style={{
          ...
          <div style={{ marginTop: 16 }}>
            ...
            <div style={{ marginBottom: 10 }}>
              <button onClick={handleDispatcherPreflight}
