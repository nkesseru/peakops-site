...
# zsh safety if user runs via zsh
setopt NO_NOMATCH 2>/dev/null || true
set +H 2>/dev/null || true

PAGE='next-app/src/app/admin/incidents/[id]/bundle/page.tsx'
BACKUP="${PAGE}.bak_bundle_v1_$(date +%Y%m%d_%H%M%S)"
...
export default function BundlePage() {
  ...
  return (
    <div style={{ minHeight: "100vh", padding: 24, background: "#0b0f19", color: "white" }}>
      ...
      {err && (
        <div style={{ marginTop: 10, color: "#ff6b6b", fontWeight: 900, whiteSpace: "pre-wrap" }}>{err}</div>
      )}
