...
export async function GET(req: Request) {
  try {
    ...
    {
      const bUrl = `${origin}/api/fn/getIncidentBundle?orgId=${encodeURIComponent(orgId)}&incidentId=${encodeURIComponent(incidentId)}`;
...
echo "==> smoke download + verify filings exist"
...
echo "--- filings files in zip ---"
unzip -l "$TMP/packet.zip" | egrep "filings/(index|dirs|oe417|nors|sar|baba)\.json|filings/_bundle_error\.json" || true

echo "--- quick sanity: filings/index.json preview ---"
unzip -p "$TMP/packet.zip" "filings/index.json" | head -n 80 || true
