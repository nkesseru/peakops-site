...
echo "==> (0) Hard kill ports + stray dev/emulators"
lsof -tiTCP:3000,5001,8080,8081,4400,4409,9150 2>/dev/null | xargs kill -9 2>/dev/null || true
pkill -f "firebase emulators:start" 2>/dev/null || true
pkill -f "next dev" 2>/dev/null || true

echo "==> (1) Build functions_emu scaffold (CJS + runtime)"
mkdir -p functions_emu/dist

# Ensure functions_emu has its own deps (so we can require('esbuild') reliably)
...
# Install deps (local to functions_emu)
...
echo "==> (3) Write functions_emu/index.js (robust default/named export resolver)"
...
\1
exports.getWorkflowV1 = onRequest(getWorkflowV1);
...
echo "==> (6) Seed contract + payloads into Firestore emulator"
node - <<NODE
...
(async () => {
  await db.collection("contracts").doc("${CONTRACT_ID}").set({
    ...
    status: "ACTIVE",
    type: "MSA",
    customerId: "${ORG_ID}" === "org_001" ? "cust_acme_001" : "cust_demo",
...
echo "==> (7) Point Next to emulator + start Next"
ENVF="next-app/.env.local"
mkdir -p next-app
grep -q '^FN_BASE=' "$ENVF" 2>/dev/null && sed -i '' 's|^FN_BASE=.*|FN_BASE='"$FN_BASE"'|g' "$ENVF" || echo "FN_BASE=$FN_BASE" >> "$ENVF"
grep -q '^NEXT_PUBLIC_DEV_DEFAULT_ORG_ID=' "$ENVF" 2>/dev/null && sed -i '' 's|^NEXT_PUBLIC_DEV_DEFAULT_ORG_ID=.*|NEXT_PUBLIC_DEV_DEFAULT_ORG_ID='"$ORG_ID"'|g' "$ENVF" || echo "NEXT_PUBLIC_DEV_DEFAULT_ORG_ID=$ORG_ID" >> "$ENVF"
...
curl -sS "$FN_BASE/exportContractPacketV1?orgId=$ORG_ID&contractId=$CONTRACT_ID&versionId=$VERSION_ID&limit=200" \
  | python3 -m json.tool | head -n 40 || true
echo

curl -fsS "http://127.0.0.1:3000/api/fn/getContractsV1?orgId=$ORG_ID&limit=5" | head -c 200; echo
curl -fsS "http://127.0.0.1:3000/api/fn/getContractV1?orgId=$ORG_ID&contractId=$CONTRACT_ID" | head -c 200; echo
