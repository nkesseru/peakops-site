...
<Panel title="Packet State (stub)">
  <div style={{ display: "grid", gap: 6, fontSize: 13, opacity: 0.9 }}>
    <div><span style={{ opacity: 0.7 }}>filingsMeta:</span> {wf?.wf?.wf?.wf?.incident?.filingsMeta ? "present" : "—"}</div>
    <div><span style={{ opacity: 0.7 }}>timelineMeta:</span> {wf?.wf?.wf?.wf?.incident?.timelineMeta ? "present" : "—"}</div>
...
{JSON.stringify(
  {
    filingsMeta: wf?.wf?.wf?.wf?.incident?.filingsMeta || null,
    timelineMeta: wf?.wf?.wf?.wf?.incident?.timelineMeta || null,
  },
  null,
  2
)}
...
<Panel title="Packet State (stub)">
          <div style={{ display:"grid", gap:6, fontSize: 13, opacity: 0.9 }}>
            <div><span style={{ opacity: 0.7 }}>filingsMeta:</span> {wf?.wf?.incident?.filingsMeta ? "✅ present" : "—"}</div>
            <div><span style={{ opacity: 0.7 }}>timelineMeta:</span> {wf?.wf?.incident?.timelineMeta ? "✅ present" : "—"}</div>
            <div style={{ fontSize: 12, opacity: 0.75 }}>
              (Stub) This becomes the canonical packet readiness panel.
            </div>
          </div>
          ...
          <details style={{ marginTop: 10 }}>
            ...
            <pre style={{ marginTop: 10, whiteSpace:"pre-wrap", fontSize: 12, opacity: 0.9 }}>
{JSON.stringify({ filingsMeta: wf?.wf?.incident?.filingsMeta || null, timelineMeta: wf?.wf?.incident?.timelineMeta || null }, null, 2)}
            </pre>
          </details>
        </Panel>
