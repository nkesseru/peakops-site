...
// IMPORTANT: [id] is at /admin/contracts/[id], so _components is TWO levels up:
...
export default function AdminContractDetail() {
  ...
  const payloadsHref = useMemo(
    () =>
      `/admin/contracts/${encodeURIComponent(contractId)}/payloads?orgId=${encodeURIComponent(
        orgId
      )}`,
    [orgId, contractId]
  );
  ...
  async function load() {
    ...
    try {
      const r = await fetch(
        `/api/fn/getContractV1?orgId=${encodeURIComponent(orgId)}&contractId=${encodeURIComponent(
          contractId
        )}`,
        { method: "GET" }
      );
      ...
      if (!parsed.ok) {
        setDoc({});
        setErr(
          `Contract API returned non-JSON (${r.status}): JSON Parse error: ${parsed.error}`
        );
        return;
      }
      ...
      // tolerate either {doc} or full object
      setDoc(j?.doc || j);
    } catch (e: any) {
      setDoc({});
    ...
    <div style={{ padding: 24, fontFamily: "system-ui", color: "CanvasText" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <Link style={pill()} href={`/admin/contracts?orgId=${encodeURIComponent(orgId)}`}>
            ← Contracts
          </Link>
          <a style={pill()} href={payloadsHref}>
            Payloads →
          </a>
          <a style={pill()} href={packetHref}>
            Preview Packet
          </a>
          <button style={pill()} onClick={downloadZip} disabled={busy}>
            Download ZIP
          </button>
          <button style={pill()} onClick={load} disabled={busy}>
            {busy ? "Loading…" : "Refresh"}
          </button>
        </div>
      </div>

      <div style={{ marginTop: 10, display: "flex", gap: 10, fontSize: 12, opacity: 0.85, flexWrap: "wrap" }}>
        <div>
          Contract # <b>{doc?.contractNumber || "—"}</b>
        </div>
        <div>
          Type <b>{doc?.type || "—"}</b>
        </div>
