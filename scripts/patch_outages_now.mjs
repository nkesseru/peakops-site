...
// 2) ensure Firestore client after app creation
...
// 3) remove any existing GET/PATCH for outages to avoid dupes
s = s.replace(/app\.get\(\s*['"]\/v1\/telecom\/outages['"][\s\S]*?\}\);\s*/g,'');
s = s.replace(/app\.patch\(\s*['"]\/v1\/telecom\/outages\/:id['"][\s\S]*?\}\);\s*/g,'');

// 4) insert canonical routes right before "export default app"
const idx=s.lastIndexOf('export default app');
if (idx===-1){ console.error('❌ missing "export default app"'); process.exit(1); }

const BLOCK = `
/* ===== Telecom outages: filtered GET + PATCH ===== */
...
console.log('✅ routes patched (GET filtered + PATCH) and Firestore import normalized');
