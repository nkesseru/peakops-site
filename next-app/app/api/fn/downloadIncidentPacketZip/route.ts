...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const timeline = await getJsonSameOrigin(req, `/api/fn/getTimelineEventsV1?orgId=${encodeURIComponent(orgId)}&incidentId=${encodeURIComponent(incidentId)}&limit=200`).catch(() => ({ ok: true, docs: [] }));
