...
function PanelCard({ title, children }: any) {
  ...
  async function loadEvidenceLocker() {
    try {
      // If you already have a global busy setter, keep it. Otherwise this no-ops.
      if (typeof setBusy === "function") setBusy(true);

      const base = (process.env.NEXT_PUBLIC_PEAKOPS_FN_BASE || "http://127.0.0.1:5001/peakops-pilot/us-central1");
      const qs = new URLSearchParams({
        orgId: String(orgId || "org_001"),
        incidentId: String(incidentId || ""),
        limit: "25",
      });

      const url = `${base}/listEvidenceLocker?${qs.toString()}`;
      const r = await fetch(url);
      const j = await r.json();

      if (!j || j.ok !== true) {
        throw new Error(j?.error || "listEvidenceLocker failed");
      }
      ...
      // If you have setErr, use it; otherwise ignore.
      if (typeof setErr === "function") setErr(String(e?.message || e));
    } finally {
...
useEffect(() => {
    ...
    loadBundle(); loadRil();
   loadEvidenceLocker(); // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [incidentId]);
  ...
  return (
    <div style={{ padding: 24, fontFamily: "system-ui", color: "CanvasText" }}>
      ...
      <div style={{ display:"flex", gap:10, marginTop:14, flexWrap:"wrap" }}>
        <Button disabled={!!busy} onClick={() => { loadBundle(); loadRil();loadEvidenceLocker(); }}>Refresh</Button>
