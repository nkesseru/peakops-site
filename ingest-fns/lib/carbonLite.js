...
async function resolveFactors(orgId, input) {
...
function calculateCO2e(input, factors) {
...
export const onCarbonEventCreated = onDocumentCreated("orgs/{orgId}/carbon_events/{eventId}", async (event) => {
    ...
    const inputs = {
        ...
        idle_fuel: data.idle_fuel ?? "unknown",
        kwh_used_on_site: data.kwh_used_on_site ?? 0,
        activity_start_iso: data.activity_start_iso,
        activity_end_iso: data.activity_end_iso,
        notes: data.notes,
    };
