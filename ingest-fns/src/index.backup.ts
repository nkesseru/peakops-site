...
const DIRS_URL = (process.env.DIRS_URL || (functions.config().dirs?.url as string) || "").trim();
const DIRS_AUTH = (process.env.DIRS_AUTH || (functions.config().dirs?.auth as string) || "").trim();
...
function jitter(n: number) { return Math.floor(n * (0.85 + Math.random() * 0.3)); }
function backoffDelay(attempt: number) { return jitter(BASE_DELAY_MS * Math.pow(2, attempt - 1)); }
...
async function logReceipt(ref: FirebaseFirestore.DocumentReference, entry: {
  system: string; status: "success" | "error"; attempt: number; httpCode?: number; receiptId?: string; responseSnippet?: string;
}) {
  await ref.update({ receipts: FieldValue.arrayUnion({ ts: Timestamp.now(), ...entry }) });
}

async function markSubmitted(ref: FirebaseFirestore.DocumentReference) {
  await ref.update({ status: "submitted", lease: { until: null, by: null }, lastError: null });
}

async function markFailed(ref: FirebaseFirestore.DocumentReference, code?: number, message?: string) {
  await ref.update({ status: "failed", lease: { until: null, by: null }, lastError: { code: code || null, message: message || "Unspecified error" } });
}

async function incrementAttempts(ref: FirebaseFirestore.DocumentReference) {
  await ref.update({ attempts: FieldValue.increment(1) });
}

async function postToFccDirs(body: unknown, idempotencyKey: string): Promise<{ ok: boolean; httpCode: number; receiptId?: string; text?: string }> {
  ...
  try {
    ...
    try { const j = JSON.parse(text); receiptId = j?.receiptId || j?.id || j?.data?.receiptId; } catch {}
...
async function handleOne(submissionId: string, runnerId: string) {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (resp.ok) { await markSubmitted(ref); return; }

    if (attempt < MAX_RETRIES) {
      await sleep(backoffDelay(attempt));
      await ref.update({ lease: { until: Timestamp.fromMillis(Timestamp.now().toMillis() + LEASE_SECONDS * 1000), by: runnerId } });
    } else {
...
async function workBatch(runnerId: string) {
  ...
  snap.forEach((doc) => {
    ...
    if (d.status === "queued" || (d.status === "processing" && expired)) candidates.push(doc.id);
  });
  const leased: string[] = [];
  await Promise.all(candidates.map(async (id) => { if (await takeLease(id, runnerId)) leased.push(id); }));
  await Promise.all(leased.map((id) => handleOne(id, runnerId)));
}

export const processQueuedSubmissions = onSchedule(
  { schedule: "*/1 * * * *", timeZone: "America/Los_Angeles", memory: "512MiB", cpu: 1, region: "us-west1", concurrency: 10 },
  async () => { const runnerId = `sched-${crypto.randomUUID().slice(0, 8)}`; await workBatch(runnerId); }
);

export const kickProcessQueuedSubmissions = onRequest(
  { region: "us-west1", invoker: "public", memory: "512MiB", cpu: 1 },
  async (_req, res) => {
